...
dotnet_project=$(find . -maxdepth 2 \( -name "*.csproj" -o -name "*.fsproj" -o -name "*.sln" \) 2>/dev/null | head -1)

if [ -n "$dotnet_project" ]; then
    project_dir=$(dirname "$dotnet_project")
    current_md5=$(find . -maxdepth 2 \( -name "*.csproj" -o -name "*.fsproj" \) -exec md5sum {} + 2>/dev/null | sort | md5sum | awk '{print $1}')
    stored_md5=""
