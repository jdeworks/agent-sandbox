...
if [ -z "$SANDBOX_PROFILE_DIR" ]; then
    ...
    echo "  See: src/scripts/unix/prepare.sh"
    exit 1
fi
...
get_agent_command() {
    local config_file="$PROJECT_DIR/sandbox_data/agent-config.json"

    local agent="opencode"
    if [ -n "${SANDBOX_AGENT_SESSION:-}" ]; then
        agent="$SANDBOX_AGENT_SESSION"
...
prompt_agent_selection() {
    ...
    local available=$(jq -r '.availableAgents[]' "$config_file" 2>/dev/null | tr '\n' ',' | sed 's/,$//')
    local current=$(jq -r '.defaultAgent' "$config_file" 2>/dev/null || echo "opencode")
    echo ""
    echo "[sandbox] Available CLI agents: $available"
    echo "[sandbox] Current default: $current"
    read -rp "Use a different agent for this session? (enter agent name or press Enter to keep $current): " choice

    if [ -n "$choice" ]; then
        if jq -e --arg choice "$choice" '.availableAgents | index($choice)' "$config_file" >/dev/null 2>&1; then
            echo "[sandbox] Selected agent for this session: $choice"
            export SANDBOX_AGENT_SESSION="$choice"
        else
            echo "[sandbox] Invalid agent '$choice'. Using default: $current"
        fi
    fi
}
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$CONTAINER_NAME"; then
    ...
    if [ "$reuse_choice" = "1" ]; then
        ...
        echo "[sandbox] Attaching to $CONTAINER_NAME (using $AGENT_CMD)..."
...
if [ ! -d "$PROJECT_DIR" ]; then
    ...
    # Copy default agent-config.json if it exists in templates
    if [ -f "$TEMPLATES_DIR/agent-config.json" ]; then
