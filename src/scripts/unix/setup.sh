...
add_alias "sandbox-cleanup-prepared" "bash $SCRIPT_DIR/sandbox-cleanup.sh prepared"
   ...
   add_alias "sandbox-cleanup-prepared-unused" "bash $SCRIPT_DIR/sandbox-cleanup.sh prepared --unused-only"
   echo "  sandbox-cleanup-prepared-unused -> remove only prepared profiles not used by any project"
...
while IFS= read -r line; do
    ...
    case "$alias_name" in
        sandbox-list|sandbox-stats|sandbox-cleanup|sandbox-cleanup-sudo|sandbox-cleanup-prepared|sandbox-cleanup-prepared-unused|sandbox-cleanup-prepared-sudo|prepare) continue ;;
