# Backend volumes API and Docker setup

The frontend expects these backend endpoints. Add them as follows.

## 1. Create `apps/backend/src/routes/volumes.ts`

Create the file with the content from the implementation below (see "volumes.ts content").

## 2. Register the route in `apps/backend/src/routes/index.ts`
...
## 3. Docker Compose: project and env for backend/worker

So the backend can read `scan-volumes.json` and list mounted volume paths, add to **backend** and **worker**:

- **Environment** (backend): `SCAN_VOLUMES_FILE: /project/scan-volumes.json`
- **Volumes** (backend and worker): `- .:/project` (in addition to `uploads:/uploads`)
...
```yaml
  backend:
    ...
    environment:
      # ... existing ...
      SCAN_VOLUMES_FILE: /project/scan-volumes.json
    volumes:
      - uploads:/uploads
      - .:/project
```

Same for worker (volumes and optional env if it ever needs to read the list).
